...
//!
//! **Ce qu'on savait en ouvrant ce fichier**, relevé avant lui et jamais refait
//! depuis : l'interpréteur x86 en Swift rendait **10,6 MIPS**, le cœur rv32 en
//! Rust **157**, et un module WebAssembly **écrit à la main** compilé par
//! WebKit **1103**. Ces trois-là sont des points d'origine, pas des faits
//! d'aujourd'hui : **aucun ne se relance depuis ce dépôt sur un coureur Linux**
//! — le cœur Swift demande la chaîne Apple, et le module écrit à la main a été
//! remplacé par ce que l'émetteur engendre. Ils sont gardés parce qu'ils disent
//! d'où l'on part, et datés parce qu'ils ne disent rien d'autre.
//!
//! Ce que le fichier cherche tient en une phrase qui, elle, ne dépend d'aucun
//! des trois : rien sans JIT ne rattrapera un module écrit à la main, mais
//! l'interpréteur en Swift n'est pas la limite de ce qu'on peut faire sans JIT.
//!
...
//! Alpine. Au 14 septembre 2026, trois passages :
//!
//! | | débit |
//! |---|---|
//! | ce cœur-ci, en Rust | **32,6 – 34,3 MIPS** |
//! | l'émetteur, registres seuls, forme libre | 313,8 – 315,4 |
//! | l'émetteur, registres seuls, **forme confinée** | 283,8 – 290,1 |
//! | l'émetteur, une lecture et une écriture, forme libre | 577,2 – 577,7 |
//! | l'émetteur, une lecture et une écriture, **forme confinée** | 408,4 – 416,3 |
//!
//! **Huit à dix-huit fois**, selon la forme et selon ce que la boucle touche.
//! Une version antérieure de ce commentaire donnait « 49 MIPS » contre
//! « 247 MIPS » et en tirait « cinq fois » : les deux nombres ne se
//! reproduisent pas, et depuis que le banc distingue la forme libre de la
//! confinée, **un seul chiffre d'émetteur ne désigne plus rien** — il y en a
//! quatre. Ce qui se transporte d'un coureur à l'autre est le rapport, pas les
//! grandeurs.
//!
//! Deux choses s'y lisent quand même, et ce sont des ordres. La première :
//! l'interpréteur en Rust rend plusieurs fois celui en Swift, sans WebView ni
//! permission. La seconde : **l'émetteur n'atteint pas le module écrit à la
//! main**, et l'écart est celui entre du code engendré et du code écrit à la
//! main — le module de l'émetteur matérialise des drapeaux et passe par une
//! boucle de répartition à chaque bloc, ce que le module écrit à la main
//! n'avait pas à faire.
//!
