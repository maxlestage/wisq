...
final class SpiceGLZWiringTests: XCTestCase {
    ...
    func testTheWindowSurvivesFromOnePumpToTheNext() async throws {
        ...
        var first = try await channel.pump(into: &surfaces, caches: &caches, streams: &streams, serial: 1, limit: 2)
        XCTAssertEqual(first.undrawable, 0, "l'image 0 se décode sans fenêtre")
        first = try await channel.pump(into: &surfaces, caches: &caches, streams: &streams, serial: 1, limit: 1)
        XCTAssertEqual(
            first.undrawable, 0,
            "l'image 1 renvoie à l'image 0 : sans fenêtre conservée, rien n'est dessiné"
        )
        XCTAssertEqual(caches.glz.count, 2, "les deux images sont dans la fenêtre")
    }
