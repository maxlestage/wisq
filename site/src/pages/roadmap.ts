      ...
      items: [
        {
          ...
          detail:
            "The work in progress, and the largest of it. iOS allows no JIT, with one exception: a WKWebView may compile WebAssembly, which is data rather than code. So a translator turns regions of x86-64 instructions into modules WebKit compiles. What is measured, on a real Alpine kernel: every one of the 10 116 entry regions a call reaches now translates, 17 of them still carrying a byte the decoder cannot read. Under JavaScriptCore the emitter runs several times faster than the Rust interpreter — `--example speed` prints both, and the ratio is what carries: the absolute figures move with the machine, which is why they are not quoted here. What is not measured: a kernel booting. A region that translates is not a region that runs. Paging is in place — a guest address goes through four levels of page tables behind a translation buffer, and the writes to CR0 and CR3 that turn it on are executed rather than refused — and page faults, software traps and the timer reach the guest. A probe in the repository, which anyone can re-run, puts a number on what translating costs: under a nanosecond per access while the buffer answers, and far more when every access lands on a fresh page — that last figure moved by a factor of five between two runs on one machine, so it is an order of magnitude and not a point.",
        },
        {
      ...
      items: [
        {
          ...
          detail:
            "Le travail en cours, et le plus gros. iOS n'autorise aucun JIT, avec une exception : un WKWebView peut compiler du WebAssembly, qui est une donnée et non du code. Un traducteur transforme donc des régions d'instructions x86-64 en modules que WebKit compile. Ce qui est mesuré, sur un vrai noyau Alpine : les 10 116 régions d'entrée qu'un `call` atteint se traduisent désormais toutes, dont 17 portent encore un octet que le décodeur ne sait pas lire. Sous JavaScriptCore l'émetteur va plusieurs fois plus vite que l'interpréteur Rust — `--example speed` imprime les deux, et c'est le rapport qui se transporte : les valeurs absolues bougent avec la machine, et c'est pourquoi elles ne sont pas citées ici. Ce qui n'est pas mesuré : un noyau qui démarre. Une région qui se traduit n'est pas une région qui s'exécute. La pagination est là — une adresse invitée traverse quatre niveaux de tables derrière un tampon de traduction, et les écritures de CR0 et CR3 qui l'allument sont exécutées plutôt que refusées — et les fautes de page, les pièges logiciels et le timer parviennent à l'invité. Une sonde, dans le dépôt et relançable, chiffre ce que coûte la traduction : moins d'une nanoseconde par accès tant que le tampon répond, et bien davantage quand chaque accès tombe sur une page neuve — ce dernier chiffre a bougé d'un facteur cinq entre deux exécutions sur une même machine, c'est donc un ordre de grandeur et pas un point.",
        },
        {
